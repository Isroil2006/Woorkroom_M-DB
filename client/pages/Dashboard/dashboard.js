...
const renderDetailView = (container, testId) => {
  ...
  const timeToggleGroup = container.querySelectorAll(
    'input[name="time-limit-toggle"]',
  );
  const timeInputFields = container.querySelector("#time-input-fields");

  timeToggleGroup.forEach((radio) => {
    radio.onchange = (e) => {
      timeInputFields.style.display =
        e.target.value === "limited" ? "flex" : "none";
    };
  });
  ...
  if (saveTimeBtn) {
    saveTimeBtn.onclick = () => {
      const isLimited =
        container.querySelector('input[name="time-limit-toggle"]:checked')
          .value === "limited";
...
const renderQuestionList = (container, testId) => {
  ...
  test.questions.forEach((q, index) => {
    ...
    const qText =
      typeof q.text === "object" ? q.text[uiLang] || q.text["uz"] : q.text;

    const answersHTML = q.answers
      .map((ans) => {
        const ansText =
          typeof ans.text === "object"
            ? ans.text[uiLang] || ans.text["uz"]
            : ans.text;
        return `
  ...
  container.querySelector("#add-question-btn").onclick = () =>
    openQuestionEditor(container, testId);
};
...
const openQuestionEditor = (container, testId, qIndex = null) => {
  ...
  if (isNew) {
    ...
    if (typeof qData.text === "string")
      qData.text = { uz: qData.text, en: "", ru: "" };
  }
  ...
  const renderEditorUI = () => {
    ...
    qData.answers.forEach((ans, i) => {
      ...
      div.querySelector(".ans-check").onclick = (e) => {
        if (qData.type !== "multiple")
          qData.answers.forEach((a) => (a.isCorrect = false));
    ...
    editView.querySelector("#q-type").onchange = (e) => {
      ...
      if (qData.type === "true-false") {
        qData.answers = [
          { text: { uz: "Rost", en: "True", ru: "Истина" }, isCorrect: true },
          {
            text: { uz: "Yolg'on", en: "False", ru: "Ложь" },
            isCorrect: false,
          },
        ];
      }
    ...
    editView.querySelector(".cancel-q-btn").onclick = () =>
      renderDetailView(container, testId);
  ...
  users = users.map((u) => {
    if (u.username === currentUser.username) {
      ...
      if (!existingRes) {
        ...
        u.totalScore = u.results.reduce(
          (sum, res) => sum + Number(res.score),
          0,
        );
      }
    }
...
export const initDashboardLogic = () => {
  ...
  const updateUI = () => {
    ...
    allTests.forEach((test, index) => {
      ...
      card
        .querySelectorAll(".dropdown-clone-btn, .dropdown-delete-btn")
        .forEach((btn) => {
  ...
  if (createBox)
    createBox.insertAdjacentHTML(
      "beforeend",
      createDashAnalyticsBtn(currentLang),
    );
...
const renderQuizView = (
  ...
  const totalSeconds =
    (parseInt(test.minutes) || 0) * 60 + (parseInt(test.seconds) || 0);
  ...
  const saveQuizState = () => {
    localStorage.setItem(
      "currentQuiz",
      JSON.stringify({ testId, examLang, userAnswers, startTime }),
    );
  };
  ...
  finishModalOverlay.onclick = (e) => {
    if (e.target === finishModalOverlay)
      finishModalOverlay.style.display = "none";
  };
    ...
    const updateTimerDisplay = () => {
      ...
      if (progressLine) {
        ...
        progressLine.style.background =
          pct < 15 ? "#b91c1c" : "linear-gradient(90deg, #ef4444, #f87171)";
      }
    };
  ...
  const showQuestion = (idx) => {
    ...
    container.querySelectorAll(".q-nav-btn").forEach((btn, i) => {
      ...
      btn.style.borderColor =
        i === idx ? "#3f8cff" : isAnswered ? "#10b981" : "#e2e8f0";
      btn.style.background =
        i === idx ? "#eff6ff" : isAnswered ? "#ecfdf5" : "white";
      btn.style.color =
        i === idx ? "#3f8cff" : isAnswered ? "#10b981" : "#64748b";
    });
    ...
    content.querySelectorAll('input[name="ans"]').forEach((inp) => {
      inp.onchange = () => {
        const selected = Array.from(
          content.querySelectorAll('input[name="ans"]:checked'),
